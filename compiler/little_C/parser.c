...
printf("11: gettoken [%s]\n", prog);

  while(iswhite(*prog) && *prog) {
printf("enter iswhite loop\n");
     ++prog;
  }
  ...
  if(*prog==EOF) { /* end of file */
    *token = EOF;
    tok = FINISHED;

printf("#token_type = Deleimiter\n");
...
printf("#token_type=delimtier\n");
  ...
  if(*prog=='"') { /* quoted string */
    ...
    prog++; *temp = '\0';
printf("#token_type=string\n");
    return(token_type=STRING);
  }
  if(isdigit(*prog)) { /* number */
    while(!isdelim(*prog)) *temp++ = *prog++;
    *temp = '\0';
printf("#token_type =number\n");
...
printf("token_type [%d], token = [%s] prog=[%s]\n", token_type, token, prog);
  return token_type;
}
